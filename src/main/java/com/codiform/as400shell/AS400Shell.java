...
public class AS400Shell {
	...
	public static void main(String[] arguments) throws IOException,
			PropertyVetoException, AS400SecurityException {
	...
	private static String getDescription(AS400 server)
			throws AS400SecurityException, IOException {
		return String.format( "%s (v%dr%dm%d) as %s", server.getSystemName(),
				server.getVersion(), server.getRelease(),
				server.getModification(), server.getUserId() );
	}
